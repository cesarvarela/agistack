...
export const messages = sqliteTable("messages", {
  ...
  toolCalls: text("tool_calls", { mode: "json" }).$type<Array<{ id: string; name: string; arguments: Record<string, unknown>; result?: unknown }>>(),
  createdAt: integer("created_at", { mode: "timestamp" }).notNull().$defaultFn(() => new Date()),
})

export const nodesRelations = relations(nodes, ({ many }) => ({ containers: many(containers) }))
export const containersRelations = relations(containers, ({ one }) => ({ node: one(nodes, { fields: [containers.nodeId], references: [nodes.id] }) }))
