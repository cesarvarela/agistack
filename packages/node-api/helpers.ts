 ...
 */
export async function* executeStreamOperation<
	TInputSchema extends z.ZodType,
	TOutputSchema extends z.ZodType,
>(
	operation: StreamOperation<TInputSchema, TOutputSchema>,
	input: z.infer<TInputSchema>,
): AsyncGenerator<OperationEvent<z.infer<TOutputSchema>>> {
...
>(
	...
	try {
		const result = await operation.execute(input, deps)
...
export function setupTerminalWebSocket(wss: WebSocketServer, secret: string) {
	...
	wss.on("connection", (ws: WebSocket, req: IncomingMessage) => {
		...
		ws.on("message", (data) => {
			wsMessageCount++
			// Convert RawData to Buffer to safely access .length and .toString()
			...
			console.log(
				`[Terminal][${sessionId}] WS->PTY #${wsMessageCount}: ${buffer.length} bytes: ${JSON.stringify(buffer.toString().substring(0, 50))}`,
			)

			try {
				ptyProcess.write(buffer.toString())
			} catch (error) {
